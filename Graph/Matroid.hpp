...
// 計算量
// set(X)：分割マトロイド O(m+n)、グラフ的マトロイド O(m+n)、横断マトロイド O(e√(m+n))
// サーキット検出：分割マトロイド O(m)、グラフ的マトロイド O(m+n)、横断マトロイド O(e+m+n)
...
struct Partition_Matroid {
    ...
    vector<int> circuit(int y) const { // C(X,y)
        int p = belong[y];
        if (cnt[p] == d[p]) {
