...
//Link-Cut Tree
//計算量 辺の追加・削除・1点更新・パスクエリ : (ならし)O(log(N))

//概要
//木をHL分解のようにいくつかのパスにして管理する。
...
struct Link_Cut_Tree{
    ...
    Link_Cut_Tree(const vector<Monoid> &v, const F &f, const Monoid &e1) : f(f), e1(e1){
        ...
        nodes.resize(n);
        for(int i = 0; i < n; i++){
            nodes[i] = make_node(i, v[i]);
        }
    }

    Node *make_node(int id, const Monoid &x){
        return new Node(id, x);
    }
    ...
    void evert(Node *t){ //tを根にする tを含む木の向きは反転する
        expose(t);
        toggle(t);
        push(t);
    }
    ...
    void link(Node *c, Node *p){ //cの親をpにするようにしてマージする
        expose(c), expose(p);
        c->par = p, p->rch = c;
    }
